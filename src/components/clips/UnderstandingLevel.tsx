...
const LEVELS: UnderstandingStatus[] = [
  "미확인",
  "이해 안됨",
  "보통",
  "완전 이해",
...
export default memo(function UnderstandingLevel({
  current,
}: UnderstandingLevelProps) {
  return (
    <View className="flex-row items-center gap-1.5">
      ...
      {LEVELS.map((level) => {
        ...
        return (
          ...
          >
            <Text
              className={`font-semibold text-xs ${isActive ? colors.text : "text-slate-400"}`}
            >
              {level}
            </Text>
          </Pressable>
