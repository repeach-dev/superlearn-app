...
import {
  headerContainer, headerLeftGroup, navGroup, iconGroup, profileButton,
  navItem, navText, iconButton, badge, badgeText, logoutText,
  ...
  return (
    <View className={headerContainer({ platform: P })} style={dragStyle}>
      <View className={headerLeftGroup({ platform: P })} style={noDragStyle}>
        <Link href="/" asChild>
          <Pressable className="flex-row items-center" accessibilityRole="link" aria-label="홈으로 이동">
            <Image
      ...
      <View className={iconGroup({ platform: P })} style={noDragStyle}>
        ...
        >
          ...
          <View className={badge({ platform: P })}>
            <Text className={badgeText({ platform: P })} style={{ fontVariant: ['tabular-nums'] }}>2</Text>
          </View>
        </Pressable>
